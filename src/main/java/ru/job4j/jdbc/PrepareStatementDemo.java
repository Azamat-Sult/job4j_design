...
public class PrepareStatementDemo {
    ...
    public City insertAndReturnID(City city) {
        try (PreparedStatement statement =
                     connection.prepareStatement("insert into cities(name, population) values (?, ?)",
    ...
    public static void main(String[] args) {
        City msk = new City(5, "Moscow", 14000000);
        City ufa = new City(6, "Ufa", 1200000);
        try {
            PrepareStatementDemo demo = new PrepareStatementDemo();
            //demo.insert(msk);

            /*demo.insertAndReturnID(ufa);
            System.out.println("returned id: " + ufa.getId());*/

            /*ufa.setPopulation(1600000);
            demo.update(ufa);*/

            /*demo.delete(5);
            demo.delete(6);*/

            List<City> rsl = demo.findAll();
            for (City city: rsl) {
